names <- tibble(
  files = list.files(
    file.path("output", "mdcev", "model_12")
  )
) %>%
  filter(
...
for (i in 1:length(names)) {
  model <- readRDS(
    file.path("output", "mdcev", "model_12", names[i])
  )
  ...
  table_alpha <- base_table %>%
    filter(type == "Baseline constant (Alpha)") %>%
    spread(
      choice,
      estimate
    )

  table_gamma <- base_table %>%
    filter(type == "Satiation parameter") %>%
    spread(
      choice,
      estimate
    )

  table_delta <- base_table %>%
  ...
  flextable(
    ...
    add_body_row(
      values = c(
        "Satiation paremeter (Alpha): 0",
        "Translation parameter (Gamma):",
        table_gamma %>%
          select(-category, -value) %>%
          select(prim, temp, home) %>%
          gather() %>%
          mutate(
            value = case_when(
              key == "prim" ~ paste0(value[1], " (Primary)", collapse = ""),
              T ~ value
            )
          ) %>%
          pull(value)
