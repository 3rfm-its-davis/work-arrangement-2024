exec_model_10_alpha_fixed <- function(day) {
  ...
  apollo_control <- list(
    modelName       = paste0("model_10_alpha_fixed_", day),
    modelDescr      = "
  Budget: 24
  Fixed: Alpha
  I.V.
  ...
  apollo_fixed <- c("sig_base", "alpha_base")
  ...
  apollo_probabilities <-
    function(apollo_beta, apollo_inputs, functionality = "estimate") {
      ...
      alpha <- list(
        ...
        prim_ampk = 1 / (1 + exp(-alpha_base)),
        temp_ampk = 1 / (1 + exp(-alpha_base)),
        home_ampk = 1 / (1 + exp(-alpha_base)),
        prim_mddy = 1 / (1 + exp(-alpha_base)),
        temp_mddy = 1 / (1 + exp(-alpha_base)),
        home_mddy = 1 / (1 + exp(-alpha_base)),
        prim_pmpk = 1 / (1 + exp(-alpha_base)),
        temp_pmpk = 1 / (1 + exp(-alpha_base)),
        home_pmpk = 1 / (1 + exp(-alpha_base)),
        prim_offt = 1 / (1 + exp(-alpha_base)),
        temp_offt = 1 / (1 + exp(-alpha_base)),
        home_offt = 1 / (1 + exp(-alpha_base))
      )

      gamma <- list(
        prim_ampk = gamma_prim_ampk,
        temp_ampk = gamma_temp_ampk,
        home_ampk = gamma_home_ampk,
        prim_mddy = gamma_prim_mddy,
        temp_mddy = gamma_temp_mddy,
        home_mddy = gamma_home_mddy,
        prim_pmpk = gamma_prim_pmpk,
        temp_pmpk = gamma_temp_pmpk,
        home_pmpk = gamma_home_pmpk,
        prim_offt = gamma_prim_offt,
        temp_offt = gamma_temp_offt,
        home_offt = gamma_home_offt
      )
